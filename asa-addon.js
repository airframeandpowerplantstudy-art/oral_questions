...
homeBlock.innerHTML=`
<div class="asa-source-head"><div><div class="asa-source-kicker">Additional source bank</div><h2 class="title" style="margin-top:4px">Dale Crane / ASA 2024 Oral Guide</h2><p class="subtitle" style="margin-bottom:0">Knowledge and risk-management questions kept separate from the original question banks.</p></div><span class="asa-badge">${totalAsa.toLocaleString()} questions</span></div>
...
function codeStudy(value,msgId){
  ...
  if(!hits.length){if(msg)msg.textContent=`No ASA questions matched “${raw}”. Check the section and subject letters.`;return}
...
function renderBanks(s){selectedSection=s;selectedBank=null;const bs=asaBanks.filter(b=>b.section===s),g=$('asaBankGrid');$('asaBanksTitle').textContent=`ASA ${sectionLabel(s)}`;g.innerHTML='';bs.forEach(b=>{const k=b.questions.filter(q=>q.category==='Knowledge').length,r=b.questions.length-k,btn=document.createElement('button');btn.className='cardbtn asa-card';btn.innerHTML=`<strong>${esc(b.title)}</strong><span>${b.questions.length} questions</span><div class="asa-meta-line"><span class="asa-badge">${k} K</span><span class="asa-badge">${r} R</span></div>`;btn.onclick=()=>renderModes(b);g.appendChild(btn)});const mix=document.createElement('button');mix.className='cardbtn mixed';mix.innerHTML=`<strong>Mixed ASA ${sectionLabel(s)} Exam</strong><span>4 questions from every subject; 3 of 4 required in each subject</span>`;mix.onclick=()=>startMixed();g.appendChild(mix);if($('headSub'))$('headSub').textContent=`ASA ${sectionLabel(s)} oral practice`;show('asaBanksScreen')}
function renderModes(b){selectedBank=b;const k=b.questions.filter(q=>q.category==='Knowledge').length,r=b.questions.length-k;$('asaModeTitle').textContent=b.title;const g=$('asaModeGrid');g.innerHTML='';[
 ['practice','Practice',`10 random questions with immediate feedback`],['complete','Complete',`Every question in this subject (${b.questions.length})`],['knowledge','Knowledge Only',`${k} knowledge questions`],['risk','Risk Management Only',`${r} risk-management questions`]
 ].forEach(([m,title,sub])=>{const btn=document.createElement('button');btn.className='cardbtn asa-card';btn.disabled=(m==='risk'&&!r);btn.innerHTML=`<strong>${title}</strong><span>${sub}</span>`;btn.onclick=()=>startSingle(m);g.appendChild(btn)});show('asaModesScreen')}
function tagged(q,b){return {...q,bankTitle:b.title,bankKey:b.key,section:b.section}}
function startSingle(m){mode=m;let pool=selectedBank.questions;if(m==='knowledge')pool=pool.filter(q=>q.category==='Knowledge');if(m==='risk')pool=pool.filter(q=>q.category==='Risk Management');questions=shuffle(pool).slice(0,m==='practice'?10:pool.length).map(q=>tagged(q,selectedBank));last={kind:'single',section:selectedSection,bankKey:selectedBank.key,mode:m};start()}
function startMixed(){mode='mixed';const bs=asaBanks.filter(b=>b.section===selectedSection);questions=shuffle(bs.flatMap(b=>shuffle(b.questions).slice(0,4).map(q=>tagged(q,b))));last={kind:'mixed',section:selectedSection};start()}
function start(){idx=0;answers=[];pending=null;attemptCounts={};if(logger?.startQuiz)try{logger.startQuiz({section:selectedSection,mode:`asa-${mode}`,bankKey:selectedBank?.key||'asa-code-or-mixed',bankTitle:selectedBank?.title||'ASA filtered/mixed',questionCount:questions.length,source:'ASA 2024'})}catch{};show('asaExamScreen');load()}
function setStatus(t,on=false){$('asaStatusText').textContent=t;$('asaStatus').classList.toggle('listening',on)}
function load(){const q=questions[idx];$('asaQNum').textContent=`Question ${idx+1} of ${questions.length}`;$('asaModeLabel').textContent=mode==='mixed'?`Mixed ASA ${sectionLabel(selectedSection)}`:mode==='code'?'ACS Code Study':mode==='practice'?'Practice':mode==='knowledge'?'Knowledge Only':mode==='risk'?'Risk Management':'Complete';$('asaAcsCode').textContent=q.acsCode;$('asaCategory').textContent=q.category;$('asaSubjectLabel').textContent=q.bankTitle;$('asaQText').textContent=q.question;$('asaAnswer').value='';finalText='';pending=null;$('asaFeedback').className='asa-feedback';$('asaSubmit').disabled=false;$('asaSubmit').textContent='Submit Answer';$('asaNext').disabled=true;if($('headStat'))$('headStat').textContent=`${idx+1} / ${questions.length}`;if($('bar'))$('bar').style.width=`${idx/questions.length*100}%`;setStatus('Begin when ready.')}
